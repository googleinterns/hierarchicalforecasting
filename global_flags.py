...
flags.DEFINE_string(
    "favorita_dir", "./data/favorita", "Path to the m5 data directory"
)
...
flags.DEFINE_integer(
    "batch_size", None, "Batch size for the randomly sampled batch"
)

flags.DEFINE_integer("cont_len", 20, "Length of the historical context")

flags.DEFINE_integer(
    "fixed_lstm_hidden", 16, "Number of LSTM hidden units in the local model"
)
flags.DEFINE_integer(
    "var_lstm_hidden", 10, "Number of LSTM hidden units in the variance model"
)
...
flags.DEFINE_integer(
    "emb_seed", None, "The random seed to be used only for the embeddings"
)
...
flags.DEFINE_boolean(
    "output_scaling", False, "Learning output scale parameters"
)
flags.DEFINE_boolean(
    "emb_as_inp", False, "Whether to provide node embeddings as input"
)
flags.DEFINE_boolean(
    "load_alternate", False, "Whether to load the alternate TS data"
)
FLAGS = flags.FLAGS
